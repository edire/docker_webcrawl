...
connection = SageIntacctSDK(
    ...
    company_id = os.getenv('company_id2'),
...
con.to_sql(df, 'tblARInvoices_E150', schema='stage', if_exists='replace', index=False)
...
con.to_sql(df, 'tblCustomers_E150', schema='stage', if_exists='replace', index=False)
...
logger.info('Done with Sage API')